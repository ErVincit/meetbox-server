...
router.post("/", async (req, res, next) => {
  ...
  if (token)
    return res.status(200).send({
      msg:
        "Sei gia stato loggato. In un implementazione futura sarai ridirezionato alla homepage.",
    }); // Already logged
...
router.post("/registration", async (req, res) => {
  ...
  try {
    const result = await loginService.registration(
      email,
      firstName,
      lastName,
      password
    ...
    return res
      .status(412)
      .send({ error: "RegistrationError", message: err.message });
  }
});
