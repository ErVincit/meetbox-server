...
router.post("/", async (req, res) => {
  try {
    ...
    if (!name)
      throw new Error(
        "E' necessario aggiungere un nome per creare un gruppo di lavoro"
      );
    if (!image)
      throw new Error(
        "E' necessario aggiungere un'immagine per creare un gruppo di lavoro"
      );
...
router.put("/:idWorkgroup/edit", async (req, res) => {
  try {
    ...
    const { name, image } = req.body;
    if (!name && !image)
      throw new Error("E' necessario aggiungere almeno un campo da modificare");
    const workgroup = await workgroupService.editWorkgroup(
      userId,
      idWorkgroup,
      name,
      image
...
router.delete("/:idWorkgroup", async (req, res) => {
  try {
    const { idWorkgroup } = req.params;
    const userId = req.currentUser;
    const workgroup = await workgroupService.deleteWorkgroup(
      userId,
      idWorkgroup
    );
    res.json({ data: workgroup });
...
router.put("/:idWorkgroup/member/:idMember", async (req, res) => {
  try {
    ...
    const userId = req.currentUser;
    const member = await workgroupService.addMember(
      userId,
      idWorkgroup,
      idMember
    );
...
router.delete("/:idWorkgroup/member/:idMember", async (req, res) => {
  try {
    const { idWorkgroup, idMember } = req.params;
    const userId = req.currentUser;
    const member = await workgroupService.removeMember(
      userId,
      idWorkgroup,
      parseInt(idMember)
    );
