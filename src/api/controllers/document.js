...
router.put("/:idDocument/edit", async (req, res, next) => {
	...
	try {
		if (members || name || folder) await documentService.edit(currentUser, idDocument, idWorkgroup, members, name, folder);
		const data = await documentService.get(currentUser, idDocument, idWorkgroup);
