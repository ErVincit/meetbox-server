    ...
    if (!document.task) {
      ...
      if (!Object.keys(listOfDirectory).includes("" + document.folder)) {
        ...
        if (tempDirectory.folder !== ROOT) {
          while (
            !Object.keys(listOfDirectory).includes("" + tempDirectory.folder)
          ) {
            ...
            if (
              ...
              listOfDirectory[tempDirectory.folder].filter(
                (d) => d.id === tempDirectory.id
              ).length === 0
            )
              listOfDirectory[tempDirectory.folder].push(tempDirectory);
          }
      ...
      if (
        listOfDirectory[document.folder].filter((d) => d.id === document.id)
          .length === 0
      )
...
exports.create = async (
  ...
  try {
    ...
    try {
      ...
      document.members = await documentService.setAllMembers(
        currentUser,
        idDocument,
        members,
        workgroup
