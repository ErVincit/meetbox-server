...
exports.getAllWorkgroups = async (userId) => {
  const results = await pool.query(
    'SELECT wg.* FROM "UserWorkGroup" uwg, "WorkGroup" wg WHERE uwg.userid = $1 AND uwg.workgroup = wg.id',
    [userId]
  );
  const data = [];
...
exports.editWorkgroup = async (userId, workgroupId, name, image) => {
  ...
  try {
    if (!this.checkWorkgroupMembers([userId], workgroupId, userId))
      throw new Error(
        "Operazione fallita. Potresti aver richiesto di accedere ad una risorsa inesistene o di cui non hai l'accesso"
      );
    let workgroup;
    if (name) {
      const result = await client.query(
        'UPDATE "WorkGroup" SET name = $1 WHERE id = $2 RETURNING *',
        [name, workgroupId]
      );
    ...
    if (image) {
      const result = await client.query(
        'UPDATE "WorkGroup" SET image = $1 WHERE id = $2 RETURNING *',
        [image, workgroupId]
      );
      workgroup = result.rows[0];
    }
...
exports.createWorkgroup = async (userId, name, image) => {
  ...
  try {
    const result = await client.query(
      'INSERT INTO "WorkGroup"(name, image, owner) VALUES ($1, $2, $3) RETURNING *',
      [name, image, userId]
    );
    const workgroup = result.rows[0];
    await client.query('INSERT INTO "UserWorkGroup" VALUES ($1, $2)', [
      userId,
      workgroup.id,
    ]);
    // Add workgroup members
...
exports.deleteWorkgroup = async (userId, workgroupId) => {
  ...
  try {
    ...
    if (workgroup.owner !== userId)
      throw new Error("Solo il proprietario del workgroup può eliminarlo");
    // Delete all members of the workgroup
    const members = await client.query(
      'SELECT userid FROM "UserWorkGroup" WHERE workgroup = $1',
      [workgroupId]
    );
    for (const member of members.rows)
      await this.removeMember(userId, workgroupId, member.userid);
    // Delete the workgroup
    const results = await client.query(
      'DELETE FROM "WorkGroup" WHERE id = $1 RETURNING *',
      [workgroupId]
    );
    client.release();
    return results.rows[0];
...
exports.addMember = async (userId, workgroupId, memberId) => {
  ...
  try {
    ...
    if (workgroup.owner !== userId)
      throw new Error(
        "Solo il proprietario del workgroup può aggiungere un membro"
      );
    // Add the new member
    await client.query('INSERT INTO "UserWorkGroup" VALUES ($1, $2)', [
      memberId,
...
exports.removeMember = async (userId, workgroupId, memberId) => {
  ...
  if (workgroup.owner !== userId && userId !== memberId)
    throw new Error(
      "Solo il proprietario del workgroup può rimuovere questo membro"
    );
  const client = await pool.connect();
  try {
    // Delete all the documents where the deleted user is the owner
    const docsResult = await client.query(
      'SELECT id FROM "Document" WHERE owner = $1 AND workgroup = $2',
      [memberId, workgroupId]
    );
    for (const doc of docsResult.rows)
      await documentService.delete(userId, doc.id, workgroupId);
    ...
    for (const userDoc of userDocResult.rows)
      await client.query(
        'DELETE FROM "UserDocument" WHERE document = $1 AND userid = $2',
        [userDoc.id, memberId]
      );
    ...
    for (const task of tasksResult.rows)
      await activityService.deleteTask(
        task.id,
        task.section,
        workgroupId,
        userId
    ...
    for (const userTask of userTasksResult.rows)
      await client.query(
        'DELETE FROM "UserTask" WHERE task = $1 AND userid = $2',
        [userTask.id, memberId]
      );

    // Delete all the events where the deleted user is the owner
    const eventsResult = await client.query(
      'SELECT id FROM "Event" WHERE owner = $1 AND workgroup = $2',
      [memberId, workgroupId]
    );
    for (const event of eventsResult.rows)
      await calendarService.deleteEvent(memberId, event.id);
    ...
    for (const userEvent of userEventsResult.rows)
      await client.query(
        'DELETE FROM "UserEvent" WHERE event = $1 AND userid = $2',
        [userEvent.id, memberId]
      );

    // Delete the member
    const result = await client.query(
      'DELETE FROM "UserWorkGroup" WHERE userid = $1 AND workgroup = $2 RETURNING *',
      [memberId, workgroupId]
    );
...
exports.getAllMembers = async (userId, workgroupId) => {
  ...
  const result = await pool.query(
    'SELECT u.* FROM "UserWorkGroup" uwg, "User" u WHERE uwg.userid = u.id AND uwg.workgroup = $1',
    [workgroupId]
  );
...
exports.checkWorkgroupMembers = async (
  membersIdToCheck,
  workgroupId,
  userId
) => {
  ...
  for (const member of membersIdToCheck)
    if (!membersId.includes(member)) return false;
  return true;
};
