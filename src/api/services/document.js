...
exports.setAllMembers = async (currentUser, documentid, members, idWorkgroup) => {
	...
	try {
		...
		for (var j = 0; j < members.length; j++) await pool.query('INSERT INTO "UserDocument" VALUES($1, $2)', [members[j], documentid]);
	} catch (err) {
		throw err;
	}
};
...
exports.delete = async (currentUser, idDocument, idWorkgroup) => {
	const deleteElement = async (document, tree) => {
		if (document.isfolder) {
			...
			if (figlioletti) for (var i = 0; i < figlioletti.length; i++) return deleteElement(figlioletti[i], tree);
			if (await isFolderEmpty(document.id, idWorkgroup)) {
				await pool.query('DELETE FROM "UserDocument" WHERE document = $1', [document.id]);
				const results = await pool.query('DELETE FROM "Document" WHERE id = $1 RETURNING *', [document.id]);
	...
	try {
		...
		if (Object.keys(exists).length === 0) throw new Error("Il documento non esiste");
		...
		if (exists.isfolder) {
			...
			return deleteElement(exists, files);
		}
		return deleteElement(exists);
	} catch (err) {
		throw err;
	}
};
...
exports.editMembers = async (currentUser, idDocument, idWorkgroup, members) => {
	...
	//Se documento fa parte dello stesso stesso workgroup di cui fai parte
	//Se puoi vedere il documento
	if (!(await this.enabledToWatch(currentUser, idDocument, idWorkgroup))) throw new Error("Accesso al documento negato");
	if (!(await workgroupServices.checkWorkgroupMembers(members, idWorkgroup, currentUser)))
...
exports.editName = async (currentUser, idDocument, idWorkgroup, name) => {
	...
	if (await this.isNameUsed(name, doc.isfolder, doc.folder)) throw new Error("Esiste gia un documento con lo stesso nome");
...
exports.editFolder = async (currentUser, idDocument, idWorkgroup, folder) => {
	...
	if (idDocument == folder) throw new Error("Non puoi spostare la cartella in se stessa");
	...
	//Verificare che non esistono altri file con lo stesso nome
	const doc = await this.get(currentUser, idDocument, idWorkgroup);
	if (await this.isNameUsed(doc.name, doc.isfolder, folder)) throw new Error("Esiste gia un documento con lo stesso nome");
	//Allora cambia padre al documento
	await pool.query(`UPDATE "Document" SET folder = $1 WHERE id = $2`, [folder, idDocument]);
};
...
exports.isNameUsed = async (name, isFolder, idFolder) => {
	try {
		const res = await pool.query(`SELECT * FROM "Document" d WHERE d.name = $1 AND d.isfolder = $2 AND d.folder = $3;`, [
			name,
			isFolder,
			idFolder,
		]);
